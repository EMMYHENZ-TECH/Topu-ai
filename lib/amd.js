...
// Improved port selection with fallback logic
const getAvailablePort = async (basePort = 3000) => {
  ...
  if (global.port) {
    const available = await checkPort(global.port);
    if (available) return global.port;
  }

  // Try sequential ports (basePort to basePort+100)
...
(async () => {
  const quickport = await getAvailablePort();
  app.listen(quickport, () => {
    console.log(`Server running on http://localhost:${quickport}/`);
    // Store port for potential reuse
    global.port = quickport; 
  }).on('error', err => {
    console.error(`Port ${quickport} failed:`, err.message);
    process.exit(1);
  });
